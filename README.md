...
The LLM step uses LangChain + a Pydantic schema so the output stays consistent: summary, market positioning, a short competitor list, and a few recommendations. Prices are passed through with their original currency so comparisons don't mix marketplaces.
...
```
amazon-price-competitor/
├── src/amazon_price_competitor/
│   ├── main.py              # Streamlit UI and app flow
│   ├── services.py          # scrape + competitor fetch/store
│   ├── oxylabs_client.py    # Oxylabs API calls and response parsing
│   ├── llm.py               # GPT-4 competitor analysis
│   ├── db.py                # TinyDB wrapper
│   └── data.json            # local product storage (created at runtime)
├── pyproject.toml
├── uv.lock
├── .python-version
└── README.md
...
**`llm.py`** — loads the product + competitors, asks GPT-4 for a structured analysis, and formats it for the UI.

**`db.py`** — insert/get/search products. Duplicate ASINs are skipped on insert.

## Setup

You need Python 3.12+, [uv](https://docs.astral.sh/uv/), an [Oxylabs](https://oxylabs.io/) account, and an OpenAI API key.

```bash
git clone <your-repo-url>
cd amazon-price-competitor
uv sync
```
...
Then run the app from the package folder:

```bash
cd src/amazon_price_competitor
...
- This is a personal/learning project, not a production tool. API usage (Oxylabs + OpenAI) can add up if you scrape a lot of products.
- Don't commit `.env` or `data.json` if they contain keys or scraped data you don't want public.
- Competitor matching is based on title + category search, so results are "similar listings," not a guaranteed same-SKU match.
